    ...
    if (category && category.category_products) {
        ...
        displayProducts.forEach((product) => {
            ...
            productCard.innerHTML = `
                ...
                <p class="${product?.badge_text? "margin" : ""} ${product.badge_text ?? 'no-badge'}">${product?.badge_text || ''}</p>
                <div class="title-line">
                    <h3 class="product-title">${product.title.substring(0, 10) + `${product.title.length > 11 ? '...' : ''}`}</h3>
                    <p class="product-vendor">&#x25CF;&nbsp;${product.vendor}</p>
                </div>
                <div class="price-line">
                    <p class="product-price">Rs ${product.price}</p>
                    <p class="product-mrp">Rs ${product.compare_at_price}</p>
